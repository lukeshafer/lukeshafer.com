...
<script lang="ts">
	...
	onMount(async () => {
		let count = 0;
		while (employerTextElement.scrollWidth > employerTextElement.clientWidth && count < 100) {
			employerFont -= 0.02;
			await tick();
			count++;
		}
		// TODO: make while loop above a function that takes in the element and font
		while (jobTitleElement.scrollWidth > jobTitleElement.clientWidth && count < 100) {
			jobTitleFont -= 0.02;
			await tick();
			count++;
		}
...
>
	...
	<p class="job-title" bind:this={jobTitleElement} style:font-size={`${jobTitleFont}em`}>
		{job.title}
	</p>
